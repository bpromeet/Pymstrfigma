...
import { UsersRound, UserPlus, UserX, UserCheck, Trash2, AlertCircle } from "lucide-react";
...
interface TeamManagementProps {
  ...
  initialNewMember?: NewMemberForm;
}

const DEFAULT_NEW_MEMBER: NewMemberForm = {
  name: "",
  email: "",
  role: "view-only"
};
...
export const TeamManagement = ({
  initialMembers = [],
  initialNewMember = DEFAULT_NEW_MEMBER,
}: TeamManagementProps) => {
  const [teamMembers, setTeamMembers] = useState<TeamMember[]>(initialMembers);
  const [showAddMember, setShowAddMember] = useState(false);
  const [newMember, setNewMember] = useState<NewMemberForm>(initialNewMember);
  ...
  return (
    <PageLayout>
                    ...
                    <Table className="table-fixed">
                      <TableHeader>
                        <TableRow className="border-b border-[#43586C] hover:bg-transparent">
                          <TableHead className="text-[#798A9B] w-[30%]">
                            Member
                          </TableHead>
                          <TableHead className="text-[#798A9B] w-[15%]">
                            Role
                          </TableHead>
                          <TableHead className="text-[#798A9B] w-[18%]">
                            Status
                          </TableHead>
                          <TableHead className="text-[#798A9B] w-[22%]">
                            Last Active
                          </TableHead>
                          <TableHead className="text-[#798A9B] w-[15%]">
                            Actions
                      ...
                      <TableBody>
                        {teamMembers.map((member) => (
                          <TableRow
                            key={member.id}
                            className="border-b border-[#43586C]/30 hover:bg-transparent"
                          >
                            {/* Member Column */}
      ...
      {/* Mobile FAB - Add Team Member 
          ⚠️ CRITICAL: Uses bottom-24 (96px) to clear bottom navigation bar
          See Guidelines.md → FAB + Bottom Navigation Conflict Prevention
      */}
      <button
        onClick={() => setShowAddMember(true)}
